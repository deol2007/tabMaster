...
export default class TransactionScreen extends React.Component {
  ...
  checkStudentEligibilityForBookIssue=async()=>{
    ...
    if(studentRef.docs.length==0){
      ...
      Alert.alert("StudentId doesn't exist in the library")
    }
    else{
  ...
  checkBookEligibilityForReturn=async()=>{
    const transactionRef=await db.collection("transations").where("bookId","==",this.state.scannedBookId).limit(1).get()
    var isStudentEligible="";
  ...
  checkStudentEligibilityForBookIssue=async()=>{
    ...
    if(studentRef.docs.length==0){
      ...
      isStudentEligible=false
      Alert.alert("This Student ID Doesn't Exist In Database")
    }
    else{
      studentRef.docs.map((doc)=>{
        var student=doc.data()
  ...
  handleTransaction = async()=>{
    ...
    if(!transactionType){
      Alert.alert("This book doesn't exist in library database")
      this.setState({
        scannedBookId:'',
        scannedStudentId:''
      })
      ...
      var isStudentEligible=await this.checkBookEligibilityForBookReturn()
      if(isStudentEligible){
